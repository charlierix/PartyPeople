...
namespace Game.Math_WPF.WPF
{
    public static class Convolutions
    {
        ...
        public static RectInt2 GetExtractRectangle(VectorInt2 imageSize, bool isSquare, double sizePercent)
        {
            ...
            // Percents
            double scaledPercent = UtilityMath.GetScaledValue_Capped(MINPERCENT + PERCENTDIFF, MAXPERCENT - PERCENTDIFF, 0, 1, sizePercent);
        ...
        public static double GetExtractScore(Convolution2D patch, VectorInt2 brightestPoint, double? idealBrightness = null)
        {
            ...
            if (patch.IsNegPos)
            {
                ...
                {
                    retVal = UtilityMath.GetScaledValue(1, 0, PERCENT_NEG_MIN, PERCENT_NEG_MAX, percent);
                }
                else
                {
                    return 0;
            ...
            else
            {
                ...
                {
                    retVal = UtilityMath.GetScaledValue(1, 0, PERCENT_POS_MIN, PERCENT_POS_MAX, percent);
                }
                else
                {
                    return 0;
            ...
            using (DrawingContext ctx = dv.RenderOpen())
            {
                for (int cntr = 0; cntr < count; cntr++)
                {
                    ...
                    while (true)
                    {
                        ...
                        radiusX = UtilityMath.GetScaledValue(minRadius, maxRadius, 0, 1, rand.NextPow(POWER));
                        radiusY = UtilityMath.GetScaledValue(minRadius, maxRadius, 0, 1, rand.NextPow(POWER));
        ...
        private static void ExtendEdge(double[] values, int width, int orthHeight, AxisFor orth, AxisFor edge)
        {
            ...
            UtilityMath.MinMax(ref edgeMidStart, ref edgeMidStop);
        ...
        private static void CopyRandomPixels(double[] values, int width, int orthIndex, int orthInc, int orthDepth, int edgeDepth, AxisFor orth, AxisFor edge, Random rand)
        {
            ...
            UtilityMath.MinMax(ref orthDepthStart, ref orthDepthStop);
    ...
    public class Convolution2D : ConvolutionBase2D
    {
        ...
        private static void ApplySoftBorder_Single(double[] values, int xFrom, int xTo, int yFrom, int yTo, bool isUp, bool useX, int width)
        {
            ...
            for (int y = yFrom; y <= yTo; y++)
            {
                ...
                if (!useX)
                {
                    percent = UtilityMath.GetScaledValue(percFrom, percTo, yFrom, yTo, y);
                }
                ...
                for (int x = xFrom; x <= xTo; x++)
                {
                    if (useX)
                    {
                        percent = UtilityMath.GetScaledValue(percFrom, percTo, xFrom, xTo, x);
                    }
        ...
        private static void ApplySoftBorder_Double(double[] values, int xFrom, int xTo, int yFrom, int yTo, bool isUpX, bool isUpY, int width)
        {
            ...
            for (int y = yFromActual; y <= yToActual; y++)
            {
                ...
                double percentY = UtilityMath.GetScaledValue(percFromY, percToY, yFrom, yTo, y);

                for (int x = xFromActual; x <= xToActual; x++)
                {
                    double percentX = UtilityMath.GetScaledValue(percFromX, percToX, xFrom, xTo, x);
